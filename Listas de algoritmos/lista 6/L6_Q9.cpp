...
void q7(){
    ...
    int n=0; 
    cout<<"Quantos clientes (max 15)? ";
    cin>>n; 
    if(n>15) n=15;
        for(int i=0;i<n;i++){ 
            cout<<"nome cpf rg numeroConta data saldo\n"; 
    ...
    cout<<"Digite 2 para buscar por CPF ou 3 para listar negativos: "; 
    int op; 
    cin>>op;
    if(op==2){ 
        string cpf; 
        ...
        for(int i=0;i<n;i++)
            if(v[i].cpf==cpf)
                cout<<v[i].nome<<" "<<v[i].cpf<<" "<<v[i].saldo<<"\n";
    }
    else if(op==3){
        for(int i=0;i<n;i++)
            if(v[i].saldo<0)
